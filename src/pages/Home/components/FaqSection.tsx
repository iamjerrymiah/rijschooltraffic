...
const faqs = [
    {
        question: 'Wat is het gemiddeld aantal rijlessen dat iemand nodig heeft?',
    ...
    {
        question: 'Als ik bij jullie begin met rijlessen moet ik dan al geslaagd zijn voor mijn theorie?',
    ...
    {
        question: 'Is het mogelijk om in de avonden en weekenden les te krijgen?',
    ...
    return (
        <Box py={10}>
            <VStack spacing={2} textAlign="center" mb={4}>
                ...
                <Text fontSize="sm" maxW="800px">Hieronder heb je een lijst met de meest voorkomende vragen.</Text>
            </VStack>
            ...
            >
                <Box flex={1} w={['100%','100%','100%','70%']}>
                    <Accordion allowToggle defaultIndex={[0]}>
                        {faqs.map((faq, i) => (
                            <AccordionItem key={i} border="1px solid #ddd" mb={2} borderRadius="md">
                                <h2>
                                    <AccordionButton _expanded={{ bg: 'gray.100' }} px={4} py={3}>
                                        <Box flex="1" textAlign="left" fontSize={'18px'}>{faq.question}</Box>
                                        <AccordionIcon />
                                    </AccordionButton>
                                </h2>
                                {faq.answer && (
                                    <AccordionPanel pb={4} px={4} fontSize="sm">{faq.answer}</AccordionPanel>
                                )}
                            </AccordionItem>
                        ))}
                    </Accordion>
                ...
                >
                    <Box>
                        <Heading size="md" mb={4}> Wat ons uniek maakt</Heading>
                        <VStack align="start" spacing={3}>
                            {uniquePoints.map((point, i) => (
                                <Flex align="center" key={i}>
                                    <Icon as={FaCheckCircle as unknown as React.ElementType} mr={2} color="green.400" />
                                    <Text fontSize="xs" color="white">{point}</Text>
                                </Flex>
                            ))}
                        </VStack>
                        <Button
                            ...
                            fontWeight={400}
                            size="sm"
                            leftIcon={<Icon as={FaCheckCircle as unknown as React.ElementType} />}
                        >
